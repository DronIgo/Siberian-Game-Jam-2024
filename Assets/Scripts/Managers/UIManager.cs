...
public class UIManager : MonoBehaviour
{
    ...
    //public void setZoomCameraPosition(Vector3 point)
    //{
    //    var view = Camera.main.WorldToViewportPoint(point);
    //    Vector2 p = new Vector2(view.x*2 - 1, view.y * 2 - 1);
    ...
    //        k = 0.9f;
    //    }
    //    p.x *= k * (canvas.pixelRect.max.x - canvas.pixelRect.min.x) / (2.0f * canvas.transform.localScale.x);
    //    p.y *= k * (canvas.pixelRect.max.y - canvas.pixelRect.min.y) / (2.0f * canvas.transform.localScale.y);
    //    ZoomCircle.transform.localPosition = p;
    //}
